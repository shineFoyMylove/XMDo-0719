    ...
    if (!recorder) {
        ...
        [recorder setRecordFinishBlock:^(XMNAudioRecorder * recorder) {
            ...
            if (self.isCancel) {
                ...
                //音频文件，传参 filePath ,secodes时长
                NSString *filePath = [recorder.filePath stringByAppendingPathComponent:recorder.filename];
                int second = abs(recorder.seconds);
                if (second != recorder.seconds) {
                    second = second +1;
                }
                
                XMNChatVoiceMessage *voiceMessage = [[XMNChatVoiceMessage alloc] initWithContent:filePath state:XMNMessageStateSending owner:XMNMessageOwnerSelf];
                voiceMessage.voiceLength = second;
                voiceMessage.voicePath = filePath;
