...
{
    void(^_cellDidSelectedBlock)(NSIndexPath *indexPath);
}
...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    self.tableView.originY = HEIGHT_NAVBAR + HEIGHT_STATUSBAR+44;  //searchBar.height
    self.tableView.height = HEIGHT_SCREEN - self.tableView.originY;
}

-(void)setFriendSesrchCellDidSelectedBlock:(void (^)(NSIndexPath *))selectedBlock{
    if (selectedBlock) {
        _cellDidSelectedBlock = selectedBlock;
    }
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_cellDidSelectedBlock) {
        _cellDidSelectedBlock(indexPath);
    }
