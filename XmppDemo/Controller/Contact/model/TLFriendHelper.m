...
- (void)addFriendWithUser:(NSArray<TLUser *> *)userArr{
    if ([userArr isKindOfClass:[NSArray class]] && userArr.count >0) {
        [self.friendsData addObjectsFromArray:userArr];
...
- (void)p_resetFriendData
{
    ...
    if (self.dataChangedBlock) {
        dispatch_async(dispatch_get_main_queue(), ^{
            weakSelf.dataChangedBlock(weakSelf.data, weakSelf.sectionHeaders, weakSelf.friendCount);
        });
    }
}
...
- (TLUserGroup *)defaultGroup
{
    if (_defaultGroup == nil) {
        ...
        item_public.remarkName = @"通讯录";
        _defaultGroup = [[TLUserGroup alloc] initWithGroupName:nil users:[NSMutableArray arrayWithObjects:item_new, item_group, item_public, nil]];   //去掉标签
    }
