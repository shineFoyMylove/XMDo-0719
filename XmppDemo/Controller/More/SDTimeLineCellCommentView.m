...
//#define TimeLineCellHighlightedColor [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1.0]
...
- (void)setupViews
{
    ...
    _bgImageView.tintColor = [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1.0];
...
//        .LeeAddTextColor(NIGHT , [UIColor grayColor]);
    ...
    for (int i = 0; i < commentItemsArray.count; i++) {
        ...
        FriCircleCommentItemModel *model = commentItemsArray[i];
...
- (void)setLikeItemsArray:(NSArray *)likeItemsArray
{
    ...
    for (int i = 0; i < likeItemsArray.count; i++) {
        FriCircleLikeItemModel *model = likeItemsArray[i];
        if (i > 0) {
...
- (NSMutableAttributedString *)generateAttributedStringWithCommentItemModel:(FriCircleCommentItemModel *)model
{
    NSString *text = model.userName;
    if (model.replyUserName.length) {
    ...
    if (model.replyUserName) {
        [attString setAttributes:@{NSLinkAttributeName : model.replyUserId} range:[text rangeOfString:model.replyUserName]];
    }
    return attString;
}

- (NSMutableAttributedString *)generateAttributedStringWithLikeItemModel:(FriCircleLikeItemModel *)model
{
    NSString *text = model.userName;
    NSMutableAttributedString *attString = [[NSMutableAttributedString alloc] initWithString:text];
