...
//    _leftImageView.contentMode = UIViewContentModeRedraw;
...
//    _centerImageView.contentMode = UIViewContentModeRedraw;
...
//        UIImage *tempImg = [UIImage imageNamed:self.imageNameArray[realIndex]];
        if ([[NSThread currentThread] isMainThread] == NO) {
            NSLog(@"支线程");
        }
        
        NSString *imgUrl = [NSString stringWithFormat:@"%@",self.imageNameArray[realIndex]];
        if ([imgUrl hasPrefix:@"http://"]) {
//            [view sd_setImageWithURL:[NSURL URLWithString:self.imageNameArray[realIndex]] placeholderImage:nil];
            [view sd_setImageWithURL:self.imageNameArray[realIndex]];
            
        }else{
            [view setImage:LOADIMAGECACHES(imgUrl)];
        }
    }
}
