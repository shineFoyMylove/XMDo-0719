...
// 图标的比例
#define ZFTabBarButtonImageRatio 0.7
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect
{
    ...
    return CGRectMake(0, titleY, titleW, titleH);   //y +2
}
 ...
 *
 *  @_param keyPath 属性名
 *  @_param object  哪个对象的属性被改变
 *  @_param change  属性发生的改变
 */
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    // 设置提醒数字的位置
    CGFloat badgeY = 5;
    CGFloat badgeX = self.frame.size.width - self.badgeButton.frame.size.width - 5;  //10
    
    if ([self.badgeButton.badgeValue isEqualToString:@"-1"]) {
        //小红点
        badgeX = self.frame.size.width - self.badgeButton.frame.size.width -12;
    }
